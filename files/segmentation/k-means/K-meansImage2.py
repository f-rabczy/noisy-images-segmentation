...
OUTPUT_FOLDER_MASK = pns.SEGMENTED + "k-means\\mask\\image2"
OUTPUT_FOLDER_SEGMENTED = pns.SEGMENTED + "k-means\\segmented\\image2"
...
def do_Kmeans(file_path, output_path):
    ...
    new_image[result_image == list[2]] = [255, 255, 255]
    new_image = cv2.cvtColor(new_image, cv2.COLOR_RGB2GRAY)
    new_image = new_image.astype(bool)
    new_image = morphology.remove_small_objects(new_image, 950)
    new_image = ndi.binary_fill_holes(new_image, np.ones((3, 3)))

    new_image = new_image.astype(np.uint8)
    new_image = cv2.morphologyEx(new_image, cv2.MORPH_ERODE, np.ones((5, 5)))
...
do_Kmeans(pns.DEFAULT_IMAGE, "\\default_image2.png")
